...
int name_def(char *name, bool flag) //查找函数名或变量名是否重复定义
{
	int i, es = 0;
	if (vartablep >= maxvartablep)
		return 21;
	for (i = vartablep - 1; i >= 0; i--) //查找是否重复定义
	{
...
int lookup(char *name, int *paddress, bool flag) //查找符号表中是否有变量定义
{
	int i, es = 0, vis = 0;
...
int program() //程序开始（其他函数和主函数）
{
	...
	fscanf(fin, "%s %s %d\n", token, token1, &Line);
	if (strcmp(token, "function") && strcmp(token, "main")) //如果不是function和main
	{
		return es = 11;
	}
	while (strcmp(token, "function") == 0) //如果是function，即其他函数
	{
		es = fun_declaration(); //调用其他函数
		if (es > 0)
			return es;
	}
	...
	es = main_declaration(); //调用main函数
	//fprintf(fout, "STOP\n");
	codes[lineno++] = (Code){"STOP", 0}; //程序停止
	if (es > 0)
		return es;
	for (int i = 0; i < vartablep; i++) //打印所有函数名变量和变量
	{
...
int fun_declaration() //其他函数
{
	varnum = 2; // var set 2 enter初始化值为2
	codes[lineno++] = (Code){"ENTER", 0}; //进行enter，等待回填
	int fun_enter = lineno - 1;
	int es = 0;
	string str;
	fscanf(fin, "%s %s %d\n", token, token1, &Line);
	if (strcmp(token, "ID")) //开始不是ID，缺少标识符
		return es = 3;
	es = name_def(token1, true); //函数名填入符号表
	for (int i = 0; i < strlen(token1); i++)
		str += token1[i]; //函数名作为map<string, int> 的key
	fscanf(fin, "%s %s %d\n", token, token1, &Line);
	if (strcmp(token, "("))
		return es = 5;
	fscanf(fin, "%s %s %d\n", token, token1, &Line);
	while (strcmp("int", token) == 0) //参数列表
	{
		fscanf(fin, "%s %s %d\n", token, token1, &Line);
		if (strcmp(token, "ID"))
			return es = 3;
	...
	if (strcmp(token, ")"))
		return es = 5;
	//	cout<<str<<endl;
	mp[str] = labelp; //为map<string, int> 的string的label赋值下标
	mp_lineo[str] = fun_enter; //map<string, int>的string赋值为函数标号开始行
	//fprintf(fout, "LABEL%d:\n", labelp++); //yep
	string tmp = "LABEL" + to_string(labelp); 
	codes[lineno++] = (Code){tmp, 0}; //函数开始label
	es = fun_body(); //函数体
	//fprintf(fout, "\tRETURN\n");
	codes[lineno++] = (Code){"RETURN", 0}; //函数返回
	codes[fun_enter].operand = varnum; //函数开始的enter进行回填值
	if (es > 0)
		return es;
...
{
	varnum = 2;
	codes[lineno++] = (Code){"ENTER", 0}; //主函数enter，回填
	...
	if (strcmp(token, ")"))
		return es = 6;
	//fprintf(fout, "LABEL0:\n");
	codes[lineno++] = (Code){"LABEL0", 0}; //主函数开始label
	es = fun_body(); //主函数体
	if (es > 0)
		return es;
	fscanf(fin, "%s %s %d\n", token, token1, &Line);
	codes[main_enter].operand = varnum; //主函数enter回填
	return es;
}
int fun_body() //函数体
{
	...
	if (strcmp(token, "{"))
		return es = 1;
	fscanf(fin, "%s %s %d\n", token, token1, &Line);
	es = declaration_list(); //变量声明 TODO
	if (es > 0)
		return es;
	es = statement_list(); //语句列表 TODO
	if (es > 0)
		return es;
	if (strcmp(token, "}"))
		return es = 2;
...
{
	int es = 0;
	while (strcmp(token, "int") == 0) //对int变量进行捕捉
	{
		es = declaration_stat(); //变量声明
		if (es > 0)
			return es;
	}
	return es;
...
int declaration_stat()
{
	...
	if (strcmp(token, "ID"))
		return es = 3;
	es = name_def(token1, false); //进行变量名定义
	if (es > 0)
		return es;
	fscanf(fin, "%s %s %d\n", token, token1, &Line); 
	//	printf("%s %s\n",token,token1);
	if (strcmp(token, ";") == 0) //声明结束
	{
		fscanf(fin, "%s %s %d\n", token, token1, &Line);
		return es;
	}
	else if (strcmp(token, ",") && strcmp(token, ";")) //下一个不是逗号或分号
		return es = 4;
	else
	{
		while (strcmp(token, ",") == 0) //多个变量声明
		{
			fscanf(fin, "%s %s %d\n", token, token1, &Line);
			if (strcmp("ID", token))
				return es = 3;
			es = name_def(token1, false); //变量名定义
			fscanf(fin, "%s %s %d\n", token, token1, &Line);
		}
		if (strcmp(token, ";") == 0) //声明结束
		{
...
int if_stat() //if语句
{
	...
	codes[lineno++] = (Code){"BRF", 0}; //if不成立跳转
	int if_cond_false = lineno - 1; //if不成立跳转等待回填
	fscanf(fin, "%s %s %d\n", token, token1, &Line);
	//	printf("%s %s\n",token,token1);
	es = statement(); //if语句成立内
	//	printf("%s %s\n",token,token1);
	if (es > 0)
		return es;
	label2 = labelp++;
	...
	//fprintf(fout, "LABEL%d:\n", label1);
	string tmp1 = "BR " + to_string(label2); //if语句成立执行完跳转出去
	string tmp2 = "LABEL" + to_string(label1); //if语句结束或else语句开始label
	codes[lineno++] = (Code){"BR", 0};
	int if_cond_true = lineno - 1;
	codes[lineno++] = (Code){tmp2, 0};
	codes[if_cond_false].operand = lineno - 1; //if语句不成立回填
	if (strcmp(token, "else") == 0) //else语句
	{
		fscanf(fin, "%s %s %d\n", token, token1, &Line);
		es = statement(); //else体
		if (es > 0)
	...
	//fprintf(fout, "LABEL%d:\n", label2);
	string tmp3 = "LABEL" + to_string(label2); //else语句结束开始label
	codes[lineno++] = (Code){tmp3, 0};
	codes[if_cond_true].operand = lineno - 1; //if语句成立执行完后跳转回填
	//	printf("%s %s\n",token,token1);
	return es;
}
int while_stat() //while语句
{
	int es = 0, label1, label2;
	label1 = labelp++;
	//fprintf(fout, "LABEL%d:\n", label1);
	string tmp1 = "LABEL" + to_string(label1); //while语句开始label
	...
	label2 = labelp++;
	//fprintf(fout, "\tBRF LABEL%d\n", label2);
	string tmp2 = "BRF " + to_string(label2); //while语句条件不成立出去，等待回填
	...
	//fprintf(fout, "\tBR LABEL%d\n", label1);
	//fprintf(fout, "LABEL%d:\n", label2);
	string tmp3 = "BR " + to_string(label1); //while体结束跳转到while开头
	codes[lineno++] = (Code){"BR", while_cond_true};
	string tmp4 = "LABEL" + to_string(label2); //while结束label
	codes[while_cond_false].operand = lineno; //while语句条件不成立，回填
	codes[lineno++] = (Code){tmp4, 0}; 
	return es;
}
int do_while_stat() //do-while语句
{
	int es = 0, label1, label2;
	label1 = labelp++;
	//fprintf(fout, "LABEL%d:\n", label1);
	string tmp1 = "LABEL" + to_string(label1); //do语句label
	codes[lineno++] = (Code){tmp1, 0};	
	int do_while_cond_true = lineno - 1; //do语句行号
	fscanf(fin, "%s %s %d\n", token, token1, &Line);
	if (strcmp(token, "{"))
		return es = 1;
	fscanf(fin, "%s %s %d\n", token, token1, &Line);
	...
	codes[lineno++] = (Code){"BRF", 0}; //while条件不成立，等待回填
	int do_while_cond_false = lineno - 1;
	//string tmp3 = "BR " + to_string(label1); //delete label
	codes[lineno++] = (Code){"BR", do_while_cond_true}; //while条件成立，回填do行号
	string tmp4 = "LABEL" + to_string(label2);
	codes[lineno++] = (Code){tmp4, 0};
	codes[do_while_cond_false].operand = lineno - 1; //while条件不成立，回填do-while结束行号
	fscanf(fin, "%s %s %d\n", token, token1, &Line);
	return es;
}
int switch_case_stat()
...
int case_list()
{
	...
	int last_case = 0;
	while (strcmp(token, "case") == 0) //所有case
	{
...
int case_stat(bool *case_false, int *last_case)
{
	...
	fscanf(fin, "%s %s %d\n", token, token1, &Line);
	//fprintf(fout, "LABEL%d:\n", labelp++);
	string tmp1 = "LABEL" + to_string(labelp-1); //case开始label
	codes[lineno++] = (Code){tmp1, 0};
	if(*case_false == true) { //条件成立，回填上一个case的跳转
	...
	if (strcmp(token, "ID"))
		return es = 3;
	if (strcmp(token1, case_id) != 0 && !flag) //如果条件不成立，留一个跳转，等待回填
	{
		//fprintf(fout, "\tBR LABEL%d\n", labelp);
...
int for_stat()
{
	...
	label2 = labelp++;
	//fprintf(fout, "\tBRF LABEL%d\n", label2); 
	codes[lineno++] = (Code){"BRF", 0}; //for第二部分不成立，等待回填
	int for_cond_false = lineno - 1;

	label3 = labelp++;
	//fprintf(fout, "\tBR LABEL%d\n", label3); 
	codes[lineno++] = (Code){"BR", 0}; //for第二部分成立，等待回填
	...
	codes[lineno++] = (Code){tmp4, 0}; //for第三部分，for执行一轮后回到这里
	...
	//codes[lineno++] = (Code){"POP", 0};
	//fprintf(fout, "\tBR LABEL%d\n", label1);
	codes[lineno++] = (Code){"BR", for_check}; //for第三部分跳转第二部分，进行检查，回填
	if (strcmp(token, ")"))
		return es = 6;
	//fprintf(fout, "LABEL%d:\n", label3);
	...
	codes[lineno++] = (Code){"BR", for_add}; //for循环体结束回到第三部分，进行操作，回填
	//fprintf(fout, "LABEL%d:\n", label2);
...
int write_stat() //wriite
{
	int es = 0;
	fscanf(fin, "%s %s %d\n", token, token1, &Line);
	es = expression(); //write语句
	if (es > 0)
		return es;
	if (strcmp(token, ";"))
		return es = 4;
...
int read_stat()
{
	...
	if (strcmp(token, "ID"))
		return es = 3;
	es = lookup(token1, &address, false); //找到变量标号
	if (es > 0)
		return es;
	//fprintf(fout, "\tIN  \n");
	codes[lineno++] = (Code){"IN", 0}; //读入
	//fprintf(fout, "\tSTO %d\n", address);
	//string tmp1 = "STO" + to_string(address);
	codes[lineno++] = (Code){"STO", address}; //存储
	//fprintf(fout, "\tPOP\n");
...
int compound_stat() //语句块
{
	int es = 0;
	fscanf(fin, "%s %s %d\n", token, token1, &Line);
	es = statement_list(); //语句列表
	if (strcmp(token, "}"))
		return es = 2;
	fscanf(fin, "%s %s %d\n", token, token1, &Line);
	return es;
...
int call_stat() //call函数
{
	...
	if (strcmp(token, "ID"))
		return es = 3;
	es = lookup(token1, &address, true); //寻找函数变量
	for (int i = 0; i < strlen(token1); i++)
		str += token1[i];
	if (es > 0)
		return es;
	...
	if (strcmp(token, ";"))
		return 4;
	//fprintf(fout, "\tBR LABEL%d\n", mp[str]);
	codes[lineno++] = (Code){"CAL", mp_lineo[str]}; //call函数，回填之前map<string, int>对应函数名的行号
	printf("call line:%d\n", lineno-1);
	fscanf(fin, "%s %s %d\n", token, token1, &Line);
	return es;
}
int expression() //表达式
{
	...
	if (strcmp(token, "ID") == 0)
	{
		fileadd = ftell(fin);
		fscanf(fin, "%s %s %d\n", token2, token3, &Line);
		if (strcmp(token2, "=") == 0) //赋值
		{
			int address;
			es = lookup(token1, &address, false); //找下一个符号
			if (es > 0)
				return es;
			fscanf(fin, "%s %s %d\n", token, token1, &Line);
			es = bool_expr(); //计算
			...
			//(fout, "\tSTO   %d\n", address);
			//string tmp1 = "STO" + to_string(address);
			codes[lineno++] = (Code){"STO", address}; //存储到变量
		}
		else
		{
			fseek(fin, fileadd, 0);
...
int factor()
{
	...
	else
	{
		...
		if (strcmp(token, "NUM") == 0) //是常数
		{
			//fprintf(fout, "\tLOADI %s\n", token1);
			//string tmp1 = token1;
			//string tmp2 = "LOADI" + tmp1;
			codes[lineno++] = (Code){"LOADI", atoi(token1)}; //字符串化为常数
			fscanf(fin, "%s %s %d\n", token, token1, &Line);
			return es;
		}
		else
