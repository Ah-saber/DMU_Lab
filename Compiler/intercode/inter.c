...
void translateExtDefList(pNode node) {
    ...
    while (node) {
        ...
        if(!strcmp(node->child->name, "CodeDec")) 
            translateCodeDec(node->child);
        else if(!strcmp(node->child->name, "MainDec"))
            translateMainDec(node->child);

        node = node->child->brother;
    }
}

void translateCodeDec(pNode node) {
    assert(node != NULL);
    ...
    if (!strcmp(node->child->brother->name, "FunDec")) {
        ...
        translateFunBody(node->child->brother->brother);
    }
}

void translateMainDec(pNode node) {
    assert(node != NULL);
    if (interError) return;
    ...
    genInterCode(IR_FUNCTION,
                 newOperand(OP_FUNCTION, newString(node->child->brother->val)));

    pItem funcItem = searchTableItem(table, node->child->brother->val);
    pFieldList temp = funcItem->field->type->u.function.argv; //取参数直接做，不用再遍历
    while (temp) {
    ...
    pNode tmp = node->child;
    while(tmp->brother) tmp = tmp->brother;
    translateFunBody(tmp);
}
...
void translateFunBody(pNode node) {
    assert(node != NULL);
    if (interError) return;
...
void translateStmt(pNode node) {
    ...
    if (!strcmp(node->child->name, "Expr")) {
        translateExp(node->child, NULL);
    }

    // Stmt -> CompSt
    else if (!strcmp(node->child->name, "FunBody")) {
        translateFunBody(node->child);
    }
...
void translateExp(pNode node, pOperand place) {
    ...
    else if (!strcmp(node->child->name, "Expr") ||
